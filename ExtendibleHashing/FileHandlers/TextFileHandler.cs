...
namespace ExtendibleHashing.FileHandlers
{
    class TextFileHandler
    {
        ...
        internal bool Read(out int blockByteSize, out List<int> blockAddresses, out List<int> blockBitDepths,
            out List<bool> fileBlockOccupacity, out int fBitDepth)
        {
            blockByteSize = -1;
            blockAddresses = null;
            blockBitDepths = null;
            fileBlockOccupacity = null;
            fBitDepth = -1;
            ...
            try
            {
                ...
                fileBlockOccupacity = lines[3].Split(Separator).Select(x => bool.Parse(x)).ToList();
                fBitDepth = int.Parse(lines[4]);
        ...
        internal void Write(int blockByteSize, List<int> blockAddresses, List<int> blockBitDepths,
            List<bool> fileBlockOccupacity, int fileBitDepth)
        {
            var linesToWrite = new string[5];
            ...
            linesToWrite[3] = string.Join(Separator.ToString(), fileBlockOccupacity);
            linesToWrite[4] = fileBitDepth.ToString();
